...
const description = elements(
  "p",
  "You may use the mouse to perform calculations or <br>just type your equation and hit type to get the answer.",
  "id",
  "description"
);
...
const equal = elements("button", "= [Enter]", "id", "equal");
div.append(
  input,
  clear,
  back,
...
document.addEventListener("keydown", (entnum) => {
  ...
  const NotAllowedKeys = /^[a-zA-Z]+$/; //Not Allowed Alphabets
  if (allowedKeys.test(entnum.key)) {
    ...
    input.value = "";
  } else  if (NotAllowedKeys.test(entnum.key)) {
    //alert("Only Numeric Values.");
  }else {
    alert("Only calucator operation is permitted.");
  }
});
